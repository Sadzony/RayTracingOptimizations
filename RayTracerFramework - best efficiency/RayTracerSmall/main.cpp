...
//with sphere pointers
void threadedRenderPntrs(const std::vector<Sphere*>& spheres, Vec3f* pImage, std::mutex* data, const int maxSubdivisions, const int thisSubdivision, unsigned const width, unsigned const height)
{
...
void SmoothScaling()
{
	...
	for (float r = 0; r <= 100; r++)
	{
		...
		for (int i = 0; i < concurrency; i++) {
			*threadList[i] = std::thread(threadedRenderPntrs, spherePool->objects, image, &data, concurrency, i, width, height);
		}
		for (int i = 0; i < concurrency; i++)
		{
			threadList[i]->join();
...
void SmoothScalingNoPool() 
{
	...
	for (float r = 0; r <= 100; r++)
	{
		...
		for (int i = 0; i < concurrency; i++) {
			*threadList[i] = std::thread(threadedRender, spheres, image, &data, concurrency, i, width, height);
		}
		for (int i = 0; i < concurrency; i++)
		{
			threadList[i]->join();
